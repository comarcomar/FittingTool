...
*/
{
	...
	//reals D[n];
	reals *D= (reals*) calloc(n, sizeof(reals));
	

	reals sum = 0.;
	reals dx, dy, r;
	
	reals LargeCircle = Ten;
...
{
	int n = data.n;
	int iter, inner, IterMAX, check_line, code;
	IterMAX = 200;
	check_line = 1;

    reals lambda;
    reals F1,F2,A11,A22,A12,dX,dY,Mxx,Myy,Mxy,Mxxy,dx,dy;
    reals d1,d2,dmin,Vx,Vy,dL1,dL2,VLx,VLy,aL,bL,R,G1,G2,sBest,gBest,AB,progress;
	dmin = One;
...
//          compute the third mixed moment (after rotation of coordinates)
    		...
    		for (int ii=0; ii<n; ii++)
    		{
    			dx = (data.X[ii] - data.meanX)*VLx + (data.Y[ii] - data.meanY)*VLy;
    			dy = (data.Y[ii] - data.meanY)*VLx - (data.X[ii] - data.meanX)*VLy;
    			Mxxy += dx*dx*dy;
    		}
